...
DROP POLICY IF EXISTS "Admins can select gallery objects" ON storage.objects;
...
CREATE POLICY "Admins can select gallery objects"
  ON storage.objects
  FOR SELECT
  TO authenticated
  USING (
    bucket_id = 'sindia'::text
    AND (storage.foldername(name))[1] = 'gallery'::text
